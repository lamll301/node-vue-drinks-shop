                ...
                <tbody class="admin-content__table-main-body">
                    <template v-if="categories.length > 0">
                        <tr class="admin-content__table-row" v-for="category in categories" :key="category._id">
                            ...
                            <td>{{ convertDate(category.deletedAt) }}</td>
                            <td>
                                <button class="fs-16 btn btn-primary" @click="onRestore(category._id)">Khôi phục</button>&nbsp;
                                <button class="fs-16 btn btn-danger" @click="onDelete(category._id)">Xóa vĩnh viễn</button>
                            </td>
                        </tr>
                    </template>
                    <template v-else>
...
export default {
    ...
    methods: {
        ...
        getAll() {
            ...
            this.$request.get(`http://localhost:8080/admin/category/trash?${params}`).then(res => {
        ...
        onDelete(categoryId) {
            ...
            if (result.isConfirmed) {
                this.$request.delete(`http://localhost:8080/admin/category/${categoryId}/force`).then(() => {
                    this.$swal.fire({
        ...
        onRestore(categoryId) {
            this.$request.patch(`http://localhost:8080/admin/category/${categoryId}/restore`).then(() => {
                this.$swal.fire({
