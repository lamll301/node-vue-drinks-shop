                ...
                <tbody class="admin-content__table-main-body">
                    <template v-if="permissions.length > 0">
                        <tr class="admin-content__table-row" v-for="permission in permissions" :key="permission._id">
                            ...
                            <td>{{ convertDate(permission.createdAt) }}</td>
                            <td>{{ convertDate(permission.updatedAt) }}</td>
                            <td>
...
export default {
    ...
    methods: {
        ...
        getAll() {
            ...
            this.$request.get(`http://localhost:8080/admin/permission?${params}`).then(res => {
        ...
        onDelete(permissionId) {
            ...
            if (result.isConfirmed) {
                this.$request.delete(`http://localhost:8080/admin/permission/${permissionId}`).then(() => {
                    this.swalFire("Xóa thành công!", "Phân quyền của bạn đã được xóa.", "success")
                    .then(() => {
                        this.getAll()
                    })
                })
