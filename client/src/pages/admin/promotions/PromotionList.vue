                ...
                <tbody class="admin-content__table-main-body">
                    <template v-if="promotions.length > 0">
                        <tr class="admin-content__table-row" v-for="promotion in promotions" :key="promotion._id">
                            ...
                            <td>{{ convertDate(promotion.createdAt) }}</td>
                            <td>{{ convertDate(promotion.updatedAt) }}</td>
                            <td>{{ statusOptions[promotion.status] || promotion.status }}</td>
                            <td>
                                <router-link :to="'/admin/promotion/edit/' + promotion._id" class="fs-16 btn btn-primary">Sửa</router-link>&nbsp;
                                <button class="fs-16 btn btn-danger" @click="onDelete(promotion._id)">Xóa</button>
                            </td>
                        </tr>
                    </template>
                    <template v-else>
...
export default {
    ...
    methods: {
        ...
        getAll() {
            ...
            this.$request.get(`http://localhost:8080/admin/promotion?${params}`).then(res => {
        ...
        onDelete(promotionId) {
            ...
            if (result.isConfirmed) {
                this.$request.delete(`http://localhost:8080/admin/promotion/${promotionId}`).then(() => {
                    this.swalFire("Xóa thành công!", "Khuyến mãi của bạn đã được xóa!", "success")
                    .then(() => {
                        this.getAll()
                    })
                })
