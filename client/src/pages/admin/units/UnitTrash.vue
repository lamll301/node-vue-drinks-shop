                ...
                <tbody class="admin-content__table-main-body">
                    <template v-if="units.length > 0">
                        <tr class="admin-content__table-row" v-for="unit in units" :key="unit._id">
                            ...
                            <td>{{ convertDate(unit.deletedAt) }}</td>
                            <td>
                                <button class="fs-16 btn btn-primary" @click="onRestore(unit._id)">Khôi phục</button>&nbsp;
                                <button class="fs-16 btn btn-danger" @click="onDelete(unit._id)">Xóa vĩnh viễn</button>
                            </td>
                        </tr>
                    </template>
                    <template v-else>
...
export default {
    ...
    methods: {
        ...
        getAll() {
            ...
            this.$request.get(`http://localhost:8080/admin/unit/trash?${params}`).then(res => {
        ...
        onDelete(unitId) {
            ...
            if (result.isConfirmed) {
                this.$request.delete(`http://localhost:8080/admin/unit/${unitId}/force`).then(() => {
                    this.swalFire("Xóa thành công!", "Đơn vị tính của bạn đã được xóa vĩnh viễn khỏi hệ thống!", "success")
                    .then(() => {
                        this.getAll()
                    })
                })
        ...
        onRestore(unitId) {
            this.$request.patch(`http://localhost:8080/admin/unit/${unitId}/restore`).then(() => {
                this.swalFire("Khôi phục thành công!", "Đơn vị tính của bạn đã được khôi phục!", "success")
                .then(() => {
                    this.getAll()
                })
            })
